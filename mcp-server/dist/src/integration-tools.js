...
const TRANSPORTS = ["SSE", "StreamableHttp"];
...
export function registerIntegrationTools(server) {
    ...
    server.registerTool("list_integrations", {
        ...
        description: "List a project's integrations grouped by kind: { pre, during (MCP servers), post, connector }. Each row has " +
    ...
    server.registerTool("create_pre_integration", {
        title: "Create pre-session webhook",
        description: "Add a PRE-session webhook (runs before a conversation; can inject context the LLM sees). " +
    ...
    server.registerTool("create_post_integration", {
        title: "Create post-session webhook",
        description: "Add a POST-session webhook (runs after a conversation ends; e.g. to deliver a transcript/result). " +
    ...
    server.registerTool("create_mcp_integration", {
        title: "Add MCP server integration",
        description: "Add an MCP server the agent can call mid-conversation (kind = during). Use load_mcp_tools first to discover " +
    ...
    server.registerTool("edit_integration", {
        title: "Edit integration (pre/during/post)",
        description: "Edit a pre/during/post integration. Full-replace PUT — read-modify-write preserves untouched fields. `kind` " +
        ...
        if (!ex)
            return textTool(`No "${a.kind}" integration with ref "${a.ref}".`, true);
        let body;
        if (a.kind === "during") {
            body = {
                name: a.name ?? ex.name,
    ...
    server.registerTool("delete_integration", {
        ...
        description: "Delete an integration of any kind. `type` is the kind selector (a wrong value deletes nothing). Irreversible — " +
    ...
    server.registerTool("list_connectors", {
        ...
        description: "List the project's MCP connector catalog + enable state: each { ref, name, description, toolSpecifications, " +
    ...
    server.registerTool("enable_connector", {
        title: "Enable MCP connector",
        description: "Enable a catalog connector with a chosen tool subset. `ref` must match a catalog ref exactly. EMPTY tool_names " +
    ...
    }, async (a) => toTool(await apiRequest("POST", `${base(a.project_id)}/integrations/connector`, {
        ref: a.ref,
        toolSpecifications: a.tool_names ?? [],
        isGlobal: a.is_global ?? null,
    })));
    server.registerTool("update_connector", {
        ...
        description: "Update an enabled connector's tool subset / global flag. FULL REPLACE — pass the COMPLETE desired tool_names " +
    ...
    }, async (a) => toTool(await apiRequest("PUT", `${base(a.project_id)}/integrations/connector?ref=${enc(a.ref)}`, {
        ref: a.ref,
        toolSpecifications: a.tool_names,
        isGlobal: a.is_global ?? null,
    })));
    server.registerTool("delete_connector", {
        ...
        description: "Disable a connector (delete IS disable — there is no separate disable). Irreversible — requires confirm:true. " +
    ...
    server.registerTool("load_mcp_tools", {
        title: "Probe an MCP server's tools",
        description: "Live-probe an MCP server URL and return its tool specifications (use the names for create_mcp_integration / " +
        ...
        if (a.auth_header_name)
            q.set("authHeaderName", a.auth_header_name);
        if (a.auth_header_value)
            q.set("authHeaderValue", a.auth_header_value);
