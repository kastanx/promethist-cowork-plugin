...
const VISUALS_BUNDLE = process.env.PROMETHIST_BUNDLE_URL ||
    "https://repository.promethist.ai/public/arc/bundles/default/index.json";
export const CAMERA_PRESETS = ["Dynamic", "Smooth", "Static"];
let bundleCache = null;
export async function fetchVisualsBundle() {
    if (bundleCache)
        return bundleCache;
    const res = await fetch(VISUALS_BUNDLE, { headers: { Accept: "application/json" } });
    if (!res.ok)
        throw new Error(`Visuals bundle returned HTTP ${res.status}`);
 ...
 */
export async function buildCompoundVisualRef(avatarRef, environmentRef) {
    if (!avatarRef)
        return null;
    let bundle;
    try {
    ...
    if (environmentRef) {
        ...
        if (eVer)
            compound += `:environment-${environmentRef}-${eVer}`;
    }
    return compound;
}
/** Catalog projection for the get_visuals tool — surfaces the exact ref to pass to edit_agent. */
export function projectCatalog(bundle, kind) {
    const proj = (arr, key) => (arr ?? [])
    ...
    if (kind === "avatar")
        return { avatar: proj(bundle.avatar, "avatarRef") };
    if (kind === "environment")
        return { environment: proj(bundle.environment, "environmentRef") };
