...
export function registerTestTools(server) {
    server.registerTool("test_agent", {
        title: "Test a Promethist agent (live chat)",
        description: "Hold ONE turn of a real text conversation with a LIVE Promethist agent on the engine, to test BEHAVIOR " +
        ...
        if (!key)
            return textTool("Provide agent_ref (+ optional revision) or agent_key — e.g. agent_ref='thomasJefferson'.", true);
        ...
        if (stored)
            headers["Cookie"] = `sessionId=${stored}`;
        const out = [];
        try {
