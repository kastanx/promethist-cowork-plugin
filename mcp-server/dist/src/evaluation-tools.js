...
const EXEC_MODES = ["ON_CONVERSATION_END", "PER_TURN"];
const LOOKBACK = ["START_TO_NOW", "LAST_4"];
const INSIGHT_TYPES = ["Bool", "Number", "Text", "TextList"];
// GET-response insight type (output) -> PUT-body insight type (input).
const OUT_TO_IN = { boolean: "Bool", number: "Number", text: "Text", list: "TextList" };
...
function toInsightInput(o) {
    ...
    const base = { type: t, name: o.name, description: o.description, prompt: o.prompt };
    if (t === "Bool") {
...
function buildInsightInput(type, p) {
    const base = { type, name: p.name, description: p.description, prompt: p.prompt };
    if (type === "Bool") {
...
function validateInsightFields(type, p) {
    if (type === "Number") {
        if (p.min_value === undefined || p.max_value === undefined)
            return "A Number insight requires min_value and max_value.";
        if (p.min_value > p.max_value)
            return "min_value must be <= max_value.";
    }
...
export function registerEvaluationTools(server) {
    ...
    const evalsPath = (pid) => `/api/v1/project/${enc(pid)}/evaluations`;
    const evalPath = (pid, eid) => `${evalsPath(pid)}/${enc(eid)}`;
    server.registerTool("list_evaluations", {
        title: "List evaluations",
        description: "List a project's evaluations, each with its output data elements (insights) and attached agents. " +
    ...
    server.registerTool("get_evaluation", {
        ...
        description: "Get one evaluation with its definition and insights (incl. each insight's ref). " +
    ...
    server.registerTool("create_evaluation", {
        title: "Create evaluation",
        description: "Create an evaluation (keep it to ONE topic). It starts with no insights — add them with add_insight " +
        ...
        if (a.description !== undefined)
            evaluation.description = a.description;
        if (a.prompt !== undefined)
            evaluation.prompt = a.prompt;
        if (a.execution_modes !== undefined)
            evaluation.executionModes = a.execution_modes;
        if (a.lookback_window !== undefined)
            evaluation.lookbackWindow = a.lookback_window;
        if (a.session_agent !== undefined)
            evaluation.sessionAgent = a.session_agent;
        if (a.inject_into_featured_agent !== undefined)
            evaluation.injectIntoFeaturedAgent = a.inject_into_featured_agent;
        if (a.user_facing !== undefined)
            evaluation.userFacing = a.user_facing;
        if (a.webhook_url !== undefined)
            evaluation.webhookUrl = a.webhook_url;
        if (a.webhook_headers !== undefined)
            evaluation.webhookHeaders = a.webhook_headers;
        return toTool(await apiRequest("POST", evalsPath(a.project_id), { evaluation, insights: [] }));
    });
    server.registerTool("edit_evaluation", {
        title: "Edit evaluation",
        description: "Edit an evaluation's DEFINITION (not its insights — use add/edit/remove_insight for those). " +
        ...
        if (a.user_facing !== undefined)
            evaluation.userFacing = a.user_facing;
        if (a.admin_facing !== undefined)
            evaluation.adminFacing = a.admin_facing;
        if (a.webhook_url !== undefined)
            evaluation.webhookUrl = a.webhook_url;
        if (a.webhook_headers !== undefined)
            evaluation.webhookHeaders = a.webhook_headers;
    ...
    server.registerTool("add_insight", {
        title: "Add insight (output data element)",
        description: "Add an output data element to an evaluation — the unit it extracts/scores. Types: Bool (yes/no), " +
        ...
        return toTool(await apiRequest("PUT", evalPath(a.project_id, a.evaluation_id), {
            evaluation: evalDefToInput(d.evaluation ?? {}),
            insights,
        }));
    });
    server.registerTool("edit_insight", {
        title: "Edit insight",
        description: "Edit an existing output data element by its ref (from get_evaluation). Type is fixed (to change type, " +
        ...
        if (idx < 0)
            return textTool(`No output data element with ref "${a.insight_ref}" in this evaluation.`, true);
        ...
        if (a.prompt !== undefined)
            mod.prompt = a.prompt;
        if (inType === "Bool") {
            if (a.true_text !== undefined)
                mod.trueText = a.true_text;
            if (a.false_text !== undefined)
                mod.falseText = a.false_text;
        }
        else if (inType === "Number") {
            if (a.min_value !== undefined)
                mod.minValue = a.min_value;
            if (a.max_value !== undefined)
                mod.maxValue = a.max_value;
            if (mod.minValue > mod.maxValue)
                return textTool("min_value must be <= max_value.", true);
        }
    ...
    server.registerTool("remove_insight", {
        title: "Remove insight",
        description: "Remove an output data element from an evaluation by ref. Data loss — requires confirm:true. " +
        ...
        const d = got.data;
        const raw = (d.insights ?? []);
        if (!raw.some((x) => x.ref === a.insight_ref))
            return textTool(`No output data element with ref "${a.insight_ref}".`, true);
    ...
    server.registerTool("delete_evaluation", {
        title: "Delete evaluation",
        description: "Permanently delete an evaluation and all its output data elements. Irreversible — requires confirm:true. " +
