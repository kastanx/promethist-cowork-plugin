...
const MM_TYPES = ["input", "webpage", "image", "video", "choice", "handover"];
const INPUT_FIELD_TYPES = ["Text", "Number", "Email", "Date"];
const TYPE_NAME = {
    input: "Input",
    webpage: "Webpage",
    image: "Image",
...
function buildNested(t, a, ex) {
    const v = (param, exKey) => (a[param] !== undefined ? a[param] : ex?.[exKey]);
    const nested = {};
    const setIf = (k, val) => {
        if (val !== undefined)
            nested[k] = val;
...
export function registerMultimodalTools(server) {
    ...
    server.registerTool("list_multimodal_interactions", {
        title: "List multimodal interactions",
        description: "List a project's interactive content (input/webpage/image/video/choice/handover). Each row's type is in " +
    ...
    server.registerTool("get_multimodal_interaction", {
    ...
    server.registerTool("create_multimodal_interaction", {
        title: "Create multimodal interaction",
        description: "Create an interactive-content element. `type` picks the shape (input/webpage/image/video/choice/handover). " +
        ...
        if (!a.name || !a.name.trim())
            return textTool("name must not be blank.", true);
    ...
    server.registerTool("edit_multimodal_interaction", {
        title: "Edit multimodal interaction",
        description: "Edit an interaction. Full-replace PUT — read-modify-write preserves untouched fields. `type` must equal the " +
        ...
        if (!ex)
            return textTool(`No multimodal interaction with ref "${a.ref}".`, true);
    ...
    server.registerTool("delete_multimodal_interaction", {
        title: "Delete multimodal interaction",
        description: "Permanently delete an interactive-content element. Irreversible — requires confirm:true. " +
