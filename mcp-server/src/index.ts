...
server.registerTool(
  "login",
  {
    ...
    description:
      "Open a browser to log in to Promethist via the web app and cache the session locally. " +
...
server.registerTool(
  "logout",
  {
    ...
    description:
      "Forget the cached Promethist session (deletes the local session cache and clears the in-memory " +
  ...
  async () => {
    const r = logout();
    const msg =
      `Logged out${r.clearedFile ? " (cleared cached session)" : " (no cached session found)"}.` +
      (r.note ? ` ${r.note}` : "");
...
server.registerTool(
  "list_tenants",
  {
    title: "List tenants",
    description:
      "List all tenants (accounts) the authenticated user can access, each with its projects " +
...
server.registerTool(
  "get_project",
  {
    title: "Get project",
    description:
      "Get full details of a single project by its ID (use list_tenants to find IDs). " +
...
server.registerTool(
  "create_agent",
  {
    title: "Create agent",
    description:
      "Create a new (draft) conversational agent in a project. Only `projectId` and `name` are " +
...
server.registerTool(
  "edit_agent",
  {
    title: "Edit agent",
    description:
      "Edit an existing agent. Resolves the LATEST revision, applies ONLY the fields you pass on top of it, and " +
  ...
  async (a) => {
    ...
    let d = head.data as Record<string, any>;
    ...
    if (ref) {
      ...
      if (revs.ok && Array.isArray(revs.data)) {
        ...
        const latest = (revs.data as Array<Record<string, any>>).find((r) => r.state !== "Archived");
        if (latest?.id && latest.id !== a.agentId) {
          targetId = latest.id;
          const got = await apiGet(`/api/v1/agents/${encodeURIComponent(targetId)}`);
          if (got.ok) d = got.data as Record<string, any>;
        }
      }
    }
    ...
    if (a.avatar_ref !== undefined || a.environment_ref !== undefined || a.camera_preset !== undefined) {
      ...
      if (cameraPreset != null && !(CAMERA_PRESETS as readonly string[]).includes(cameraPreset)) {
        return {
          content: [
            {
              type: "text" as const,
