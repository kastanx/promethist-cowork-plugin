...
function qs(params: Record<string, string | number | undefined>): string {
  ...
  for (const [k, v] of Object.entries(params)) if (v !== undefined && v !== "") sp.append(k, String(v));
  const s = sp.toString();
  return s ? `?${s}` : "";
}
...
function filterQs(a: Record<string, any>, withState = false): string {
  return qs({
...
export function registerAnalyticsTools(server: McpServer) {
  const aBase = (pid: string) => `/api/v1/project/${enc(pid)}/analytics`;
  const tBase = (tid: string) => `/api/v1/tenant/${enc(tid)}`;
  ...
  server.registerTool(
    "get_conversation_analytics",
    {
      ...
      description:
        "Session & conversation counts for a project (total sessions + change %, conversations where the user spoke, " +
  ...
  server.registerTool(
    "get_usage_analytics",
    {
      ...
      description:
        "Conversation length / usage: total & average length (in SECONDS — divide by 60 for minutes) + change %, " +
  ...
  server.registerTool(
    "get_user_analytics",
    {
      ...
      description:
        "Active-user metrics: MAU/WAU/DAU (+ change %), unique users for the period, lifetime. " +
  ...
  server.registerTool(
    "get_relational_analytics",
    {
      ...
      description:
        "LLM-judge relational + business scores: userEngagement, userEmpowerment, taskCompletion, " +
  ...
  server.registerTool(
    "get_multimodal_analytics",
    {
      ...
      description:
        "Per interactive-content element: how often it was shown/executed, parameter counts, action counts. " +
  ...
  server.registerTool(
    "get_evaluation_analytics",
    {
      ...
      description:
        "Aggregated evaluation results across sessions: booleans (pass/fail counts), numbers (avg/min/max), texts " +
  ...
  server.registerTool(
    "get_extractor_analytics",
    {
      ...
      description:
        "Per custom-extractor trends over time. NOTE: this endpoint is a POST that performs a READ (returns 201, does " +
  ...
  server.registerTool(
    "list_analytics_users",
    {
      ...
      description:
        "Paged list of end-users seen in a project (keyset paging — pass the returned nextOffset back as offset). " +
    ...
    async (a) =>
      toTool(await apiGet(`${aBase(a.project_id)}/users/list${qs({ query: a.query, offset: a.offset, limit: a.limit })}`)),
  );
  ...
  server.registerTool(
    "get_analytics_summary",
    {
      ...
      description:
        "One-shot 'how is my agent doing' snapshot — fans out to conversation, usage, user, relational, multimodal, " +
  ...
  server.registerTool(
    "export_analytics",
    {
      ...
      description:
        "Export per-session analytics for a window. Use format='json' to read it in context (xlsx/csv come back as " +
  ...
  server.registerTool(
    "get_subscription",
    {
      ...
      description:
        "Get a tenant's subscription { id, status (active/past_due/canceled), description (plan) }. Read-only. " +
  ...
  server.registerTool(
    "list_invoices",
    {
      title: "List invoices",
      description:
        "List a tenant's invoices { id, status, total (in the currency's MINOR unit — divide by 100), currency, " +
  ...
  server.registerTool(
    "get_usage",
    {
      ...
      description:
        "Per-day conversation count and MINUTES (the conversational-minute billing unit) for a window. Both from & to " +
