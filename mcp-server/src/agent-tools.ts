...
export function registerAgentTools(server: McpServer) {
  ...
  server.registerTool(
    "get_guide",
    {
      ...
      description:
        "Load detailed Promethist reference guidance on demand (the in-app copilot's authoring playbook). " +
  ...
  server.registerTool(
    "get_agent",
    {
      title: "Get agent",
      description:
        "Get full detail of an agent by its UUID (name, title, description, state, kind, purpose, " +
  ...
  server.registerTool(
    "list_agents",
    {
      title: "List agents",
      description:
        "List the latest revision of each agent in a project (id, ref, name, state, ...). Use to find an agent's " +
  ...
  server.registerTool(
    "get_agent_revisions",
    {
      ...
      description:
        "List all revisions of an agent (newest first) with revision number, state " +
    ...
    async ({ agent_ref, agent_id }) => {
      ...
      if (!ref) {
        ...
        ref = (got.data as { ref?: string }).ref;
  ...
  server.registerTool(
    "get_voices",
    {
      title: "Get voices",
      description:
        "List available voice / realtime configurations (id, ref, title, description, gender, locales, " +
  ...
  server.registerTool(
    "get_visuals",
    {
      title: "Get visuals",
      description:
        'List available avatars and environments (the visual catalog). Pass an item\'s avatarRef / ' +
    ...
    async ({ type }) => {
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify(projectCatalog(bundle, type), null, 2) }] };
      } catch (e) {
        return textTool(`Failed to fetch visuals bundle: ${(e as Error).message}`, true);
      }
    },
  );
  ...
  server.registerTool(
    "list_templates",
    {
      ...
      description:
        "List published agent templates (catalog): id, ref, name, title, description, kind, variables, and the " +
  ...
  server.registerTool(
    "revert_agent",
    {
      title: "Revert agent",
      description:
        "Revert an agent to a previous revision. Call get_agent_revisions first. The server only allows reverting " +
  ...
  server.registerTool(
    "promote_agent_to_preview",
    {
      ...
      description:
        "Promote a Draft agent to Preview (available to internal testers). Only valid from Draft — check state via " +
  ...
  server.registerTool(
    "promote_agent_to_published",
    {
      ...
      description:
        "Publish an agent revision — makes it LIVE for all end users. Only valid from Preview (promote to Preview " +
