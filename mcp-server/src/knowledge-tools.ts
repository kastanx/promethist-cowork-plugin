...
import { z } from "zod";
import { apiGet, apiRequest, type ApiResult } from "./client.js";
...
function summariseIngest(r: ApiResult) {
  if (!r.ok) return toTool(r);
  const text = typeof r.data === "string" ? r.data : JSON.stringify(r.data);
  const lines = text.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
  let last: { stage?: string; ref?: string; message?: string } | null = null;
...
export function registerKnowledgeTools(server: McpServer) {
  ...
  server.registerTool(
    "list_knowledge",
    {
      ...
      description:
        "List a project's knowledge sources (url/file/static), with their metadata. " +
  ...
  server.registerTool(
    "get_knowledge_spec",
    {
      ...
      description:
        "Get one knowledge source's metadata (name, type, isGlobal, topK, similarity, language). " +
  ...
  server.registerTool(
    "get_knowledge",
    {
      ...
      description:
        "Get a knowledge source's full content and chunks (the text an agent retrieves via RAG). Use this to " +
  ...
  server.registerTool(
    "add_web_knowledge",
    {
      ...
      description:
        "Ingest a single PUBLIC web page as a knowledge source (no crawling; not for login-protected/app-like " +
  ...
  server.registerTool(
    "refresh_web_knowledge",
    {
      ...
      description:
        "Re-scrape an existing URL knowledge source. Full-object body — pass url/name (and flags) to avoid resetting " +
  ...
  server.registerTool(
    "edit_knowledge",
    {
      ...
      description:
        "Edit a knowledge source's metadata (name/description/global). Read-modify-write — preserves " +
  ...
  server.registerTool(
    "delete_knowledge",
    {
      ...
      description:
        "Permanently delete a knowledge source and all its content. Irreversible — requires confirm:true. " +
