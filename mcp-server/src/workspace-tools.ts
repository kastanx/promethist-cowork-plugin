...
const tBase = (id: string) => `/api/v1/tenant/${enc(id)}`;
const pBase = (id: string) => `/api/v1/project/${enc(id)}`;
...
export function registerWorkspaceTools(server: McpServer) {
  ...
  server.registerTool(
    "get_tenant_context",
    {
      ...
      description:
        "Get a tenant plus the CALLER's role on it ({ id, name, userRole }). Use to check if create/edit/member ops " +
  ...
  server.registerTool(
    "create_tenant",
    {
      title: "Create tenant",
      description:
        "Create a new tenant/account (any authenticated user). Note: it is seeded with a default project. " +
        "POST /api/v1/tenants.",
  ...
  server.registerTool(
    "edit_tenant",
    {
      title: "Edit tenant",
      description:
        "Edit a tenant's name/logo. Read-modify-write (the PATCH is full-replace, so it re-sends current values for " +
    ...
    async (a) => {
      ...
      const ex = got.data as Record<string, any>;
  ...
  server.registerTool(
    "get_project_context",
    {
      ...
      description:
        "Get a project plus the caller's project role AND parent-account role ({ id, name, tenantId, userRole, " +
  ...
  server.registerTool(
    "list_project_users",
    {
      ...
      description:
        "List users who have conversation SESSIONS in a project (NOT the team roster — use list_project_members for " +
  ...
  server.registerTool(
    "create_project",
    {
      ...
      description:
        "Create a project in a tenant. Requires EDITOR on the TENANT (a project-only role can't create projects). " +
    ...
    async (a) => {
      const body: Record<string, any> = { name: a.name };
      if (a.description !== undefined) body.description = a.description;
      if (a.logo !== undefined) body.logo = a.logo;
      if (a.primary_color !== undefined) body.primaryColor = a.primary_color;
      if (a.secondary_color !== undefined) body.secondaryColor = a.secondary_color;
      if (a.featured_agent_ref !== undefined) body.featuredAgentRef = a.featured_agent_ref;
  ...
  server.registerTool(
    "edit_project",
    {
      title: "Edit project",
      description:
        "Edit a project's metadata. Read-modify-write (PATCH is full-replace). WARNING: renaming re-derives the " +
  ...
  server.registerTool(
    ...
    async (a) =>
      toTool(await apiRequest("PUT", `${tBase(a.tenant_id)}/members/${enc(a.user_id)}/role`, { role: a.role })),
  );
  ...
  server.registerTool(
    "remove_tenant_member",
    {
      ...
      description:
        "Remove a member from the account (revokes their ACL + any pending account invite). Requires owner. " +
  ...
  server.registerTool(
    "list_project_members",
    {
      ...
      description:
        "List a project's members + roles (tenant members appear inherited). Requires owner. " +
  ...
  server.registerTool(
    "set_project_member_role",
    {
      ...
      description:
        "Set a member's project role. Requires owner. Note: setting a project role LOWER than the user's tenant role " +
    ...
    async (a) =>
      toTool(await apiRequest("PUT", `${pBase(a.project_id)}/members/${enc(a.user_id)}/role`, { role: a.role })),
  );
  ...
  server.registerTool(
    "remove_project_member",
    {
      ...
      description:
        "Remove a member from the project (revokes their project ACL + any pending project invite). Requires owner. " +
  ...
  server.registerTool(
    "list_tenant_invitations",
    {
      ...
      description:
        "List pending/accepted account invitations. Requires editor. GET /api/v1/tenant/{id}/invitations.",
      inputSchema: { tenant_id: z.string().describe("Tenant ID.") },
    },
  ...
  server.registerTool(
    "invite_members_to_tenant",
    {
      ...
      description:
        "Invite one or more people to the account by email (each gets a Pending invite + accept link). Requires owner. " +
    ...
    async (a) =>
      toTool(await apiRequest("POST", `${tBase(a.tenant_id)}/invitations`, { emails: a.emails, role: a.role })),
  );
  ...
  server.registerTool(
    "invite_members_to_project",
    {
      ...
      description:
        "Invite one or more people to a project by email. Requires owner. POST /api/v1/project/{id}/invitations.",
      inputSchema: {
        project_id: z.string().describe("Project ID."),
    ...
    async (a) =>
      toTool(await apiRequest("POST", `${pBase(a.project_id)}/invitations`, { emails: a.emails, role: a.role })),
  );
  ...
  server.registerTool(
    "revoke_tenant_invitation",
    {
      ...
      description:
        "Permanently revoke an account invitation. Requires owner. Irreversible — requires confirm:true. " +
  ...
  server.registerTool(
    "revoke_project_invitation",
    {
      ...
      description:
        "Permanently revoke a project invitation. Requires owner. Irreversible — requires confirm:true. " +
